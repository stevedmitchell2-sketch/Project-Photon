...
def tint(actor, color, emissive_scale=0.0):
    ...
    mid.set_vector_parameter_value("Color", c)
    mid.set_vector_parameter_value("BaseColor", c)


def box(loc, scale, label, labels, color=None, emissive=0.0):
    if label in labels:
    ...
    if color:
        tint(a, color, emissive)
    labels.add(label)
    return a


def pedestal(loc, label, labels, color):
    if label in labels:
        return None
    a = unreal.EditorLevelLibrary.spawn_actor_from_class(
        unreal.StaticMeshActor, unreal.Vector(*loc), unreal.Rotator(roll=0.0, pitch=0.0, yaw=0.0))
    ...
    a.static_mesh_component.set_mobility(unreal.ComponentMobility.STATIC)
    tint(a, unreal.LinearColor(color.r * 0.45, color.g * 0.45, color.b * 0.45, 1.0), 0.15)
    labels.add(label)
    return a
...
box((1300, -1200, 160), (5, 5, 0.35), "ArenaElevated_Deck", labels, WALL)
box((1050, -1200, 80), (2.5, 5, 0.25), "ArenaElevated_Ramp", labels, WALL)
...
for team, x, y, yaw in spawns:
    box((x, y, -45), (6, 6, 0.06), "ArenaSpawn_%s" % team, labels, TEAM[team], 0.2)
...
for i, (x, y) in enumerate(target_positions):
    pedestal((x, y, 90), "ArenaTargetPedestal_%d" % i, labels, NEON)
    if tcls:
